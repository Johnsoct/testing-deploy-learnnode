...
function autocomplete(input, latInput, lngInput) {
  ...
  var dropdown = new google.maps.places.Autocomplete(input); // Autocompletes via google maps within #address input

  dropdown.addListener('place_changed', function () {
    var place = dropdown.getPlace(); // google maps api object returning object on the location
...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); // fetching ajax
  ...
  _axios2.default.get('/api/v1/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
          ...
          placeLat = _place$location$coord[1]; // creates 2 variables
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/v1/search?q=' + this.value).then(function (res) {
      ...
      if (res.data.length) {
        var html = searchResultsHTML(res.data); // unnecessary
  ...
  searchInput.on('keyup', function (e) {
    ...
    // set .active
    var active = 'search__result--active';
    // find current active
    var current = search.querySelector('.' + active);
...
// DON'T HAVE TO LOOP OVER EACH FORM WITH BLING.JS .ON
